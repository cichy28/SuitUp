...
const ProductsScreen: React.FC<ProductsScreenProps> = ({ navigation }) => {
	...
	const loadProducts = async () => {
		try {
			...
			Alert.alert("Błąd", "Nie udało się załadować produktów. Sprawdź połączenie z internetem.");
		} finally {
			setLoading(false);
		}
	};
		...
		<View style={styles.container}>
			...
			{products.length > 0 ? (
				<FlatList
					...
					renderItem={({ item }) => (
						<Card variant="outlined" style={styles.productCard} onPress={() => handleProductPress(item.id)}>
				...
				<EmptyState
					...
					description={
						selectedCategory ? "Nie znaleziono produktów w tej kategorii" : "Nie znaleziono żadnych produktów"
					}
