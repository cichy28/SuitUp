...
function Table<T>({
	...
	const renderHeader = () => (
		<StyledView className={headerClasses}>
			{columns.map((column, index) => (
				<StyledView key={index} className={cellClasses} style={{ width: column.width || `${100 / columns.length}%` }}>
				...
				return (
					<StyledView key={index} className={cellClasses} style={{ width: column.width || `${100 / columns.length}%` }}>
						{column.render ? (
							...
							<StyledText className="text-textPrimary">{item[key as keyof T]?.toString() || ""}</StyledText>
						)}
					</StyledView>
				);
			})}
	...
	return (
		<StyledView className={containerClasses}>
			{renderHeader()}
			<FlatList data={data} renderItem={renderRow} keyExtractor={keyExtractor} />
		</StyledView>
	);
}
