...
const NUMBER_OF_PRODUCTS = 5; // Generate 5 new products
// --------------------

const bodyShapes: Array<z.infer<typeof BodyShape>> = ["INVERTED_TRIANGLE", "HOURGLASS", "OVAL", "RECTANGLE", "TRIANGLE"];
const stylePreferences: Array<z.infer<typeof StylePreference>> = ["FITTED_WEAR", "OVERSIZE_WEAR", "RETRO_SHAPES", "MASCULINE_SHAPES"];
...
async function generateProducts() {
    ...
    for (let i = 1; i <= NUMBER_OF_PRODUCTS; i++) {
        ...
        await fs.writeFile(path.join(productPath, 'product_metadata.json'), JSON.stringify(metadata, null, 2));
        console.log(`Generated product_metadata.json for ${productName}: suitableFor=${suitableFor}, style=${style}, basePrice=${basePrice}, description=${description}, isActive=${isActive}, skus=${metadata.skus.length}, properties=${metadata.properties.length}`);
    }
