    ...
    const category = await prisma.category.findUnique({
      where: { id },
       include: {
            parent: true,
            children: true,
            products: { // Include products in this category
                include: {
...
export const updateCategory = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.code === 'P2025') { // Prisma error code for record not found
     ...
     if (error.code === 'P2003') { // Invalid parentId
         return res.status(400).json({ message: 'Invalid parent category ID' });
     }
...
export const deleteCategory = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // P2003 might occur if there are related records with Restrict onDelete
     if (error.code === 'P2003') {
         return res.status(409).json({ message: 'Cannot delete category because it is referenced by other records (e.g., products or subcategories)' });
     }
