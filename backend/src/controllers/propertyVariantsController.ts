...
import {
	CreatePropertyVariantInputSchema,
	UpdatePropertyVariantInputSchema,
} from "../../../shared/validators/propertyVariant";
import { z } from "zod";
	...
	try {
		const propertyVariants = await prisma.propertyVariant.findMany({
			include: {
				...
				image: {
					// Include related Multimedia (image)
					select: { id: true, url: true, altText: true }, // Select specific multimedia fields
				},
				productSkus: {
					// Include linked ProductSkuPropertyVariant records
					include: {
