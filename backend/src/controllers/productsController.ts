...
export const getAllProducts = async (req: Request, res: Response) => {
  try {
    const products = await prisma.product.findMany({
      include: { // Optionally include related data, e.g., owner name, main image URL
        owner: {
  ...
  try {
    const product = await prisma.product.findUnique({
      ...
      include: {
        ...
        categories: { // Include categories the product belongs to
           include: {
            category: true // Include category details
           }
        },
        properties: { // Include product-specific properties
            include: {
