...
// Assuming you have validators for Property
  ...
  try {
    const properties = await prisma.property.findMany({
      include: {
        ...
        products: { // Include products associated with this property
           include: {
...
export const createProperty = async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json(newProperty);
     ...
     if (error.code === 'P2003') { // Invalid ownerId
...
export const deleteProperty = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // P2003 might occur if there's a Restrict onDelete somewhere else (less likely with Cascade)
     if (error.code === 'P2003') {
         return res.status(409).json({ message: 'Cannot delete property because it is referenced by other records with Restrict onDelete' });
     }
