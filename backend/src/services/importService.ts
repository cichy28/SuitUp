...
// --- CONFIGURATION ---
const SOURCE_FOLDER = process.env.IMPORT_PATH || "/app/backend/_do_importu";
const DESTINATION_FOLDER = process.env.UPLOAD_PATH || "/app/uploads";
...
export async function runMassImport() {
	console.log("--- Starting Mass Import via API ---");
	await fs.mkdir(DESTINATION_FOLDER, { recursive: true });
	try {
